...
```
cmd/es-cli/          程序入口
internal/cmd/        CLI 参数定义、启动流程、version/exec 子命令
...
internal/formatter/  输出格式化（repl: table/json, cmd: json/csv/yaml/go-template）
...
| 模块 | 指南 | 一句话说明 |
|------|------|-----------|
| cmd | [cmd.md](cmd.md) | 入口 + CLI 参数 + version/exec 子命令（cobra） |
| version | — | 版本信息（internal/version/，ldflags 注入） |
...
| formatter | [formatter.md](formatter.md) | 输出格式化（repl: table/json, cmd: json/csv/yaml/go-template） |
